...
// Define the validation schema for the withdrawal form
...
const WithdrawPage = () => {
    ...
    const [loading, setLoading] = useState(false); // Loading state for the withdrawal transaction
    ...
    const onSubmit = async (data) => {
        ...
        try {
            // Call the withdraw service from walletService
            const response = await walletService.withdraw(data.amount);
            // Assuming response.balance is returned from the backend
            setSuccess(`Successfully withdrew ${data.amount.toFixed(2)}. New balance: ${response.balance.toFixed(2)}`);
            // Optionally, you might want to reset the form after success
            // reset();
        } catch (err) {
            // Display error message from the backend (e.g., "Insufficient funds") or a default one
            setError(err);
        } finally {
